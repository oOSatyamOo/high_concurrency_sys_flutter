...
class RaidService {
  ...
  static const String _raidCollection = 'events';

  /// The document ID for the currently active raid.
  static const String _activeRaidDocument = 'dragon_raid';
  ...
  // Lock to serialize concurrent requests locally (simulating server-side serialization)
  // since fake_cloud_firestore lacks proper optimistic concurrency control in tests.
  Future<void>? _lock;

  Future<bool> joinRaid({required String userId}) async {
    // Wait for the previous transaction to finish before starting a new one.
    while (_lock != null) {
      await _lock;
    }
    ...
    try {
      ...
      return await _firestore.runTransaction<bool>((Transaction transaction) async {
      ...
      final List<String> participants =
          List<String>.from((data['participants'] as List<Object?>?) ?? <Object?>[]);
      ...
      // We track slots_filled explicitly for the test, but fallback to length if needed.
      final int slotsFilled = (data['slots_filled'] as num?)?.toInt() ?? participants.length;
      ...
      if (slotsFilled >= maxParticipants) {
        return false;
      }
  ...
  Stream<List<String>> watchParticipants() {
    return _firestore
      ...
      final Map<String, Object?> data = snapshot.data()!;
      return List<String>.from((data['participants'] as List<Object?>?) ?? <Object?>[]);
    });
  }
